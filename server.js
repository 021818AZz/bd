...
// Rota de registro escola (pública)
app.post('/registerescola', async (req, res) => {
    try {
        const { username, full_name, password, password_confirmation } = req.body;
        
        // ✅ VALIDAÇÃO PARA REGISTRO ESCOLA
        if (!username || !full_name || !password || !password_confirmation) {
            return res.status(400).json({
                success: false,
                message: 'Nome de usuário, nome completo da turma, senha e confirmação são obrigatórios'
            });
        }

        // Validar tamanho do username (4 a 70 caracteres)
        const usernameTrimmed = username.trim();
        if (usernameTrimmed.length < 4 || usernameTrimmed.length > 70) {
            return res.status(400).json({
                success: false,
                message: 'O nome de usuário deve ter entre 4 e 70 caracteres'
            });
        }

        // Validar nome completo (mínimo 2 caracteres)
        const fullNameTrimmed = full_name.trim();
        if (fullNameTrimmed.length < 2) {
            return res.status(400).json({
                success: false,
                message: 'O nome completo da turma deve ter pelo menos 2 caracteres'
            });
        }
        ...
        const createdDate = new Date(newUser.created_at);
        const formattedDate = createdDate.toLocaleDateString('pt-BR');
        const formattedTime = createdDate.toLocaleTimeString('pt-BR');

        res.status(201).json({
            success: true,
            message: 'Usuário escola cadastrado com sucesso',
            data: {
                id: newUser.id,
        ...
        console.error('Erro no registro escola:', error);
...
// novos DA LUJAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
// ==============================================
// NOVA ROTA: Confirmar pagamento após receber produto
// ==============================================

app.post('/api/v2/purchase/:id/confirm-received-and-pay', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const { id } = req.params;

        console.log('=== 💳 CONFIRMAÇÃO DE PAGAMENTO V2 ===');
        console.log('Usuário:', userId);
        ...
        if (!purchase) {
            console.log('❌ Compra não encontrada:', id);
            return res.status(404).json({
                success: false,
                message: 'Compra não encontrada'
            });
        ...
        if (purchase.user_id !== userId) {
            console.log('❌ Acesso negado:', userId, 'tentou acessar compra de', purchase.user_id);
            return res.status(403).json({
                success: false,
                message: 'Acesso negado: Esta compra não pertence a você'
            });
        }

        // Verificar se já está pago
        if (purchase.payment_status === 'paid') {
            console.log('⚠️ Compra já está paga:', id);
            return res.status(400).json({
                success: false,
                message: 'Esta compra já está paga'
            });
        }

        // Verificar se status é pendente
        if (purchase.payment_status !== 'pending') {
            console.log('⚠️ Status inválido:', purchase.payment_status);
            return res.status(400).json({
                success: false,
                message: 'Esta compra não está aguardando pagamento'
            });
        }

        // Verificar saldo do usuário
        console.log('📊 Verificando saldo...');
        console.log('Saldo atual:', purchase.user.saldo, 'KZ');
        console.log('Valor da compra:', purchase.amount, 'KZ');

        if (purchase.user.saldo < purchase.amount) {
            console.log('❌ Saldo insuficiente');
            return res.status(400).json({
                success: false,
                message: `Saldo insuficiente para pagar. Saldo atual: ${purchase.user.saldo} KZ, Valor necessário: ${purchase.amount} KZ`
            });
        }
        ...
        await prisma.$transaction(async (tx) => {
            ...
            console.log('✅ Saldo após pagamento:', updatedUser.saldo);

            // 2. Atualizar status de pagamento da compra
            await tx.purchase.update({
                where: { id: id },
                data: {
                    payment_status: 'paid',
            ...
            await tx.transaction.create({
                data: {
                    ...
                    amount: -purchase.amount,
                    description: `Pagamento confirmado: ${purchase.product_name}`,
                    balance_after: updatedUser.saldo,
                    created_at: new Date()
                }
            });
            ...
            // 4. Distribuir bônus (agora que foi pago)
            console.log('🎁 Distribuindo bônus...');
            await distributeReferralBonuses(tx, userId, purchase.amount);

            // 5. Registrar log
            await tx.systemLog.create({
                data: {
                    action: 'PAYMENT_CONFIRMED_V2',
                    description: `V2: ${purchase.user.mobile} confirmou pagamento da compra ${id}. Valor: ${purchase.amount} KZ. Saldo após: ${updatedUser.saldo} KZ`,
                    user_id: userId,
                    created_at: new Date()
                }
            });
        ...
        res.json({
            success: true,
            message: '✅ Pagamento confirmado com sucesso! O valor foi deduzido do seu saldo.',
            data: {
                purchase_id: id,
                amount_paid: purchase.amount,
                product_name: purchase.product_name,
                payment_status: 'paid',
                note: 'Bônus foram distribuídos para sua rede de indicações.'
            }
        });

    } catch (error) {
        console.error('❌ ERRO ao confirmar pagamento V2:', error);
        res.status(500).json({
            success: false,
            message: 'Erro interno do servidor V2: ' + error.message
        });
    }
});

// ==============================================
// NOVA ROTA: Histórico de compras com status de pagamento
// ==============================================

app.get('/api/v2/user/my-purchases', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const { page = 1, limit = 20, payment_status, show_pending } = req.query;
        
        console.log('=== 📋 HISTÓRICO DE COMPRAS V2 ===');
        console.log('Usuário:', userId);
        console.log('Filtros:', { page, limit, payment_status, show_pending });
        ...
        if (show_pending === 'true') {
            whereClause.payment_status = 'pending';
        }
        ...
        const formattedPurchases = purchases.map(purchase => ({
            ...
            quantity: purchase.quantity,
            daily_return: purchase.daily_return,
            total_investment: purchase.amount * purchase.quantity,
            cycle_days: purchase.cycle_days,
            purchase_date: purchase.purchase_date,
            next_payout: purchase.next_payout,
            expiry_date: purchase.expiry_date,
            ...
            payment_status_text: getPaymentStatusTextV2(purchase.payment_status),
            payment_method_text: getPaymentMethodTextV2(purchase.payment_method),
            total_earned: purchase.total_earned,
            payout_count: purchase.payout_count,
            last_payout: purchase.last_payout,
            days_remaining: Math.max(0, Math.ceil((new Date(purchase.expiry_date) - new Date()) / (1000 * 60 * 60 * 24))),
            can_confirm_payment: purchase.payment_status === 'pending' && purchase.status === 'active',
            created_at: purchase.created_at,
            updated_at: purchase.updated_at,
            
            // Informações adicionais
            is_paid: purchase.payment_status === 'paid',
            is_pending: purchase.payment_status === 'pending',
            needs_payment: purchase.payment_status === 'pending',
            payment_button_text: purchase.payment_status === 'pending' ? 'Confirmar Pagamento' : 'Já Pago'
        }));
        ...
        const totalPurchases = await prisma.purchase.count({ where: { user_id: userId } });
        ...
        const totalPending = await prisma.purchase.aggregate({
            where: { 
                user_id: userId,
                payment_status: 'pending'
            },
        ...
        res.json({
            ...
            data: {
                ...
                summary: {
                    ...
                    percentage_paid: totalPurchases > 0 ? Math.round((paidPurchases / totalPurchases) * 100) : 0
                },
                pagination: {
                    page: parseInt(page),
                    limit: parseInt(limit),
...
function getPaymentStatusTextV2(status) {
    const statusMap = {
        'paid': '✅ Já Pago',
...
function getPaymentMethodTextV2(method) {
    const methodMap = {
        'balance': 'Saldo da Conta',
        'post_delivery': 'Pagar Após Entrega',
        'use_balance': 'Usar Saldo',
...
// ==============================================
// NOVA ROTA: Estatísticas de pagamento do usuário
// ==============================================

app.get('/api/v2/user/payment-stats', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        
        console.log('=== 📊 ESTATÍSTICAS DE PAGAMENTO V2 ===');
        console.log('Usuário:', userId);
        ...
        const paidPurchases = purchases.filter(p => p.payment_status === 'paid').length;
        ...
        const recentPurchases = purchases.filter(p => new Date(p.purchase_date) > thirtyDaysAgo);
        const recentPaid = recentPurchases.filter(p => p.payment_status === 'paid').length;
        const recentPending = recentPurchases.filter(p => p.payment_status === 'pending').length;
        ...
        res.json({
            ...
            data: {
                overview: {
                    ...
                    paid_purchases: paidPurchases,
                    pending_purchases: pendingPurchases,
                    payment_ratio: totalPurchases > 0 ? Math.round((paidPurchases / totalPurchases) * 100) : 0
                },
                amounts: {
                    total_amount: totalAmount,
                ...
                recent_activity: {
                    last_30_days: recentPurchases.length,
                    paid_recent: recentPaid,
                    pending_recent: recentPending,
                    conversion_rate: recentPurchases.length > 0 ? Math.round((recentPaid / recentPurchases.length) * 100) : 0
                },
                recommendations: pendingPurchases > 0 ? [
                    `Você tem ${pendingPurchases} compra(s) aguardando pagamento`,
                    `Valor pendente: KZ ${pendingAmount.toLocaleString('pt-BR')}`,
                    'Confirme o pagamento após receber os produtos'
                ] : [
                    'Todas suas compras estão pagas!',
                    'Continue acompanhando seus rendimentos'
                ]
            }
        });

    } catch (error) {
        console.error('❌ ERRO nas estatísticas V2:', error);
        res.status(500).json({
            success: false,
            message: 'Erro interno do servidor V2'
        });
    }
});
